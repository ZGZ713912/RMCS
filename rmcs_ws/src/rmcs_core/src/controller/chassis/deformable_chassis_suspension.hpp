...
public:
    ...
    void configure(rclcpp::Node& node) {
        ...
        passive_kp_ = node.get_parameter_or("passive_suspension_kp", 0.035);
        passive_ki_ = node.get_parameter_or("passive_suspension_ki", 0.005);
        passive_deadband_ = std::max(node.get_parameter_or("passive_suspension_deadband", 0.08), 0.0);
        passive_integral_limit_ = std::max(
            node.get_parameter_or("passive_suspension_integral_limit", 3.0), 0.0);
        passive_load_lpf_alpha_ = std::clamp(
            node.get_parameter_or("passive_suspension_load_lpf_alpha", 0.08), 0.0, 1.0);
    ...
    Corrections update(
        ...
        const std::array<double, 4>& base_joint_angles, const std::array<double, 4>& joint_torques,
        double dt) {
        ...
        if (passive_suspension_active && passive_enabled_) {
            ...
            run_correction_trajectory_(
                ...
                base_joint_angles, passive_correction_vel_limit_, passive_correction_acc_limit_, dt,
                corrections);
            return corrections;
        }
...
private:
    ...
    void update_passive_targets_(const std::array<double, 4>& joint_torques, double dt) {
        ...
        for (size_t i = 0; i < kJointCount; ++i) {
            ...
            if (!passive_load_initialized_) {
                ...
                passive_filtered_load_proxy_[i] += passive_load_lpf_alpha_
                    * (load_proxy[i] - passive_filtered_load_proxy_[i]);
            }
        }
        ...
        for (size_t i = 0; i < kJointCount; ++i) {
            ...
            passive_load_integral_[i] = std::clamp(
                passive_load_integral_[i] + load_error * dt,
                -passive_integral_limit_, passive_integral_limit_);
        ...
        for (size_t i = 0; i < kJointCount; ++i) {
            correction_target_rad_[i] = std::clamp(
                raw_targets[i] - raw_target_mean,
                -passive_max_correction_rad_, passive_max_correction_rad_);
        }
    }
